...
function rawReq(body: string, secret?: string): Request {
  return new Request('https://example.com/api/telegram', {
    method: 'POST',
    headers: secret ? { 'X-Telegram-Bot-Api-Secret-Token': secret } : {},
    body,
...
describe('handleWebhook', () => {
  it('без правильного секрета отвечает 401 и не трогает бота', async () => {
    ...
    const res = await handleWebhook(req({ update_id: 1 }), {
      db, bot: async () => bot as never, secret: 'S',
    })
  ...
  it('обрабатывает апдейт один раз, повтор проглатывает', async () => {
    const db = await testDb()
    const bot = { handleUpdate: vi.fn() }
    const deps = { db, bot: async () => bot as never, secret: 'S' }
  ...
  it('исключение в обработчике всё равно даёт 200', async () => {
    ...
    const res = await handleWebhook(req({ update_id: 9 }, 'S'), {
      db, bot: async () => bot as never, secret: 'S',
    })
    expect(res.status).toBe(200)
  })

  it('битый JSON в теле даёт 200 и до бота не доходит', async () => {
    const db = await testDb()
    const bot = { handleUpdate: vi.fn() }
    const res = await handleWebhook(rawReq('не json', 'S'), {
      db, bot: async () => bot as never, secret: 'S',
    })
    expect(res.status).toBe(200)
    expect(bot.handleUpdate).not.toHaveBeenCalled()
  ...
  it('падение claimUpdate даёт 500', async () => {
    const db = await testDb()
    const failingDb = { ...db, q: vi.fn().mockRejectedValue(new Error('db down')) }
    const bot = { handleUpdate: vi.fn() }
    const res = await handleWebhook(req({ update_id: 7 }, 'S'), {
      db: failingDb as never, bot: async () => bot as never, secret: 'S',
    })
